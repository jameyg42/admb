    ...
    const search = Rx.of([]).pipe(
        RxOp.flatMap(_ => {
            ...
            return appSvc(ctx.client)
                   .fetchAllApps()
                   .then(apps => apps.filter(app => glob.matches(appc[1], app.name)))
        }),
        ...
        RxOp.flatMap(apps => 
            ...
            Promise.all(apps
                ...
                .map(([app,path]) => metricsExSvc(ctx.client).fetchMetrics(app, path, ctx.defaultRange, appc[2]))
